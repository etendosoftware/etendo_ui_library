  ...
  const loadOptions = async (isNewSearch = false) => {
    ...
    try {
      if (searchQuery.trim() && fetchData?.search) {
        fetchedOptions = await fetchData?.search(
          searchQuery,
          nextPage,
          pageSize,
        );
      ...
      if (page === 0 && staticData.length > 0) {
        setOptions(prevOptions =>
          isNewSearch
            ? [...staticData, ...fetchedOptions]
            : [...prevOptions, ...staticData, ...fetchedOptions],
        );
      } else {
        setOptions(prevOptions =>
          isNewSearch ? fetchedOptions : [...prevOptions, ...fetchedOptions],
        );
      }
